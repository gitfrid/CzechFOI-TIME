...
def main():
    ...
    for age_band in age_bands:                   
        ...
        for i, (name1, name2) in enumerate(pairs):
            ...
            # Recurrence plot for first timeseries
            recurrence_threshold = plt["recurrence_threshold"]  
            rp_matrix1 = recurrence_plot(filtered_df1, recurrence_threshold)

            # Recurrence plot for second timeseries
            rp_matrix2 = recurrence_plot(filtered_df2, recurrence_threshold)

            # Recurrence Plots plotten
            fig.add_trace(go.Heatmap(z=rp_matrix1, colorscale='Viridis', name=f'Recurrence Plot {name1}', showscale=False))
            fig.add_trace(go.Heatmap(z=rp_matrix2, colorscale='Viridis', name=f'Recurrence Plot {name2}', showscale=False))       
        ...
        num_cumulative_traces = 6  # Adjust if this number changes
