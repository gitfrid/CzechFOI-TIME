...
def main():
    ...
    for age_band in age_bands:  
        ...
        for i in range(0,len(dataframes_dvd)):            
            ...
            filtered_cumulative_dataframes_dvd = cumulative_dataframes_dvd[i][cumulative_dataframes_dvd[i].iloc[:, 0] >= 800]
            ...
            fig.add_trace(go.Scatter(x=filtered_cumulative_dataframes_dvd.iloc[:, 0], y=filtered_cumulative_dataframes_dvd[age_band], mode='lines', line=dict(dash='solid'), name=f'cumulative {os.path.splitext(os.path.basename(csv_files_dvd[i]))[0]}'), secondary_y=True)
