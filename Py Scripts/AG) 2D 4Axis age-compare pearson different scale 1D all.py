...
def main():
    ...
    plt = {   
        "title_text": "rolling perason for 1st Derivate different scale",         
    ...
    # name/legend of the trace must correspond with names in the pair list    
    pairs = [
    ...
    # Color palette for the standard traces 
    ...
    for age_band_pair in age_band_compare:    
        ...
        for idx, age_band in enumerate(age_band_pair):  # Loop through each individual age band in the tuple
            ...
            for i in range(0, len(dataframes_dvd)):                
                ...
                # debug print(f'pair_nr = {pair_nr} idx = {idx} dvd i = {i} {os.path.splitext(os.path.basename(csv_files_dvd[i]))[0][4:]}')                
                ...
                fig.add_trace(go.Scatter(
                    ...
                    name=f'{os.path.splitext(os.path.basename(csv_files_dvd[i]))[0][4:]} {age_band.split('-')[0]}'
                ), secondary_y=False)
                ...
                fig.add_trace(go.Scatter(
                    ...
                    name=f'Avg {os.path.splitext(os.path.basename(csv_files_dvd[i]))[0][4:]} {age_band.split('-')[0]}',
                    showlegend=True
                ), secondary_y=False)
                ...
                fig.add_trace(go.Scatter(
                    ...
                    name=f'Avg 1D {os.path.splitext(os.path.basename(csv_files_dvd[i]))[0][4:]} {age_band.split('-')[0]}',
                    showlegend=True
                ...
                fig.add_trace(go.Scatter(
                    ...
                    name=f'Avg 2D {os.path.splitext(os.path.basename(csv_files_dvd[i]))[0][4:]} {age_band.split('-')[0]}',
                    showlegend=True
                ), secondary_y=True)
                ...
                fig.add_trace(go.Scatter(
                    ...
                    name=f'cum {os.path.splitext(os.path.basename(csv_files_dvd[i]))[0][4:]} {age_band.split('-')[0]}'
                ), secondary_y=True)
                ...
                fig.add_trace(go.Scatter(
                    ...
                    name=f'{os.path.splitext(os.path.basename(csv_files_vd[i]))[0][4:]} {age_band.split('-')[0]}'
                ), secondary_y=False)            
                ...
                fig.add_trace(go.Scatter(
                    ...
                    name=f'Avg {os.path.splitext(os.path.basename(csv_files_vd[i]))[0][4:]} {age_band.split('-')[0]}',
                    showlegend=True
                ), secondary_y=False)
                ...
                fig.add_trace(go.Scatter(
                    ...
                    name=f'Avg 1D {os.path.splitext(os.path.basename(csv_files_vd[i]))[0][4:]} {age_band.split('-')[0]}',
                    showlegend=True
                ), secondary_y=True)  
                ...
                fig.add_trace(go.Scatter(
                    ...
                    name=f'Avg 2D {os.path.splitext(os.path.basename(csv_files_vd[i]))[0][4:]} {age_band.split('-')[0]}',
                    showlegend=True
                ), secondary_y=True)
                ...
                fig.add_trace(go.Scatter(
                    ...
                    name=f'cum {os.path.splitext(os.path.basename(csv_files_vd[i]))[0][4:]} {age_band.split('-')[0]}'
                ), secondary_y=True)
                # debug print(f'Tr = {len(fig.data)}')

            # the plotly color palette has exact 10 colors 
            # the additional corrlelation curves (legend entries) are aranged in 10 columnes
            # so each columne has a differnt color, for the legend rows the color stays the same 
            # to get 10 colors for second age group i use Plotly_r            
            if idx == 0:  # First age band
                color_subset = px.colors.qualitative.Plotly
            elif idx == 1:  # Second age band
                color_subset = px.colors.qualitative.Plotly_r
            ...
            for n, (name1, name2) in enumerate(pairs):
                try:
                    # Modify the names with age_band before accessing the data
                    # name/legend of trace must correspond with names of pair list
                    name1 = f"{name1} {age_band.split('-')[0]}"
                    name2 = f"{name2} {age_band.split('-')[0]}"
                    df1 = data[name1]
                    df2 = data[name2]
                ...
                fig.add_trace(go.Scatter(
                    ...
                    y=corr_values,
                    mode='lines',
                    line=dict(color=color_subset[n % len(color_subset)], width=1),
                    name=f'COR {name1}<br>{name2} {age_band.split('-')[0]}'
                ), secondary_y=True)
                # print(f'corr i = {i} color : {n}')             

            # Add five dummy traces (placeholders) to fill up legend row                
            for j in range(5):
                fig.add_trace(go.Scatter(
                    x=[None],  # Empty data, no points will be shown
                    y=[None],  # Empty data
                    mode='markers',  # No actual plot markers, just a legend entry
                    name=f'-',  # Placeholder names in the legend
                    showlegend=True,  # This will display the dummy trace in the legend
                    marker=dict(color='rgba(255, 255, 255, 0)')  # Invisible points, no markers
                ))
            ...
            fig.update_layout(
                ...
                yaxis7=dict(title='Rolling Pearson Correlation y7', overlaying='y', side='right', position=0.8,autorange=True), # Pearson y7
                #xaxis7=dict(title='Time', overlaying='x', side='bottom', position=0.5),  # Add this line for recurrence plot x-axis                
    	    	yaxis8=dict(title='',overlaying="y", side='left',autorange=True),   # title=f'{plo["yaxis1_title"]} y1'             
                ...
                yaxis12=dict(title='', overlaying="y", side="left", position=0.15,autorange=True),  # title='1st and 2nd Derivative y5 DVD'
                yaxis13=dict(title='', overlaying="y", side="left", position=0.25,autorange=True),   # title='1st and 2nd Derivative y6 VD'
                #yaxis14=dict(title='Rolling Pearson Correlation y7', overlaying='y', side='right', position=0.8,autorange=True), # Pearson y7
            ...
            if idx == 0:  # First age band - Use y1 to y7                
                num_additional_traces1 = len(fig.data) - 110 # 10 traces * 11 csv files first ag_band
                start = 0
                end = len(fig.data) - num_additional_traces1

                for j in range(start, end):  
                    # debug print(f"Trace1A {idx}: {j}: {fig.data[j].name}")   
                    if j % 10 == 0 or j % 10 == 1:      # y1                    
                            fig.data[j].update(yaxis='y1')       
                    elif j % 10 == 2 or j % 10 == 3:    
                        fig.data[j].update(yaxis='y5')  # First and second derivative of DVD
                    elif j % 10 == 4:  # y3
                        fig.data[j].update(yaxis='y3')    
                    elif j % 10 == 5 or j % 10 == 6:    # y2
                        # debug print(f"y2 {idx}: {j}: {fig.data[j].name}") 
                        fig.data[j].update(yaxis='y2')       
                    elif j % 10 == 7 or j % 10 == 8:
                        fig.data[j].update(yaxis='y6')  # First and second derivative of VD
                    elif j % 10 == 9:  # y4
                        fig.data[j].update(yaxis='y4')

                # Additional correlation traces for the first age band should use y7
                for i in range(1, num_additional_traces1 + 1):
                    # debug print(f"Trace2A {idx}: {-i}: {fig.data[-i].name}")       
                    fig.data[-i].update(yaxis='y7')  # Hardcoded for y7

            elif idx == 1: 
                num_additional_traces2 = len(fig.data) - (220 + num_additional_traces1)  # 10 traces * 22 csv files second ageband
                start = 110 + num_additional_traces1
                end = len(fig.data) - num_additional_traces2

                for j in range(start, end):  
                    # debug print(f"Trace1A {idx}: {j}: {fig.data[j].name}")   
                    if j % 10 == 0 or j % 10 == 1:      # y1                    
                            fig.data[j].update(yaxis='y8')       
                    elif j % 10 == 2 or j % 10 == 3:    
                        fig.data[j].update(yaxis='y12')  # First and second derivative of DVD
                    elif j % 10 == 4:  # y3
                        fig.data[j].update(yaxis='y10')    
                    elif j % 10 == 5 or j % 10 == 6:    # y2
                        # debug print(f"y2 {idx}: {j}: {fig.data[j].name}") 
                        fig.data[j].update(yaxis='y9')       
                    elif j % 10 == 7 or j % 10 == 8:
                        fig.data[j].update(yaxis='y13')  # First and second derivative of VD
                    elif j % 10 == 9:  # y4
                        fig.data[j].update(yaxis='y11')

                # Additional correlation traces for the first age band should use same axis y7
        ...
        print(f"Plot {html_file_path} has been saved to HTML file.")
