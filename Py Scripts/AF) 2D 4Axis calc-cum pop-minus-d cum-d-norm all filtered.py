...
def main():
    ...
    plot_name = "PVT_NUM_D PVT_NUM_VD CUM NORM"
    ...
    for i, df in enumerate(dataframes_vd):
        ...
        if i == 0:
            ...
            print(f"This is the {i+1} dataframe.")
    ...
    # Deaths normalized and then cumulated
    cum_dataframes_dvd = []
    ...
    for age_band in age_bands:  
        ...
        for i in range(0,len(dataframes_dvd)):            
            ...
            filtered_dataframes_dvd = dataframes_dvd[i][dataframes_dvd[i].iloc[:, 0] >= 800]
            filtered_dataframes_vd = dataframes_vd[i][dataframes_vd[i].iloc[:, 0] >= 800]
            filtered_cumulat_dataframes_dvd = cumulat_dataframes_dvd[i][cumulat_dataframes_dvd[i].iloc[:, 0] >= 800]
            filtered_cumulative_dataframes_vd = cumulative_dataframes_vd[i][cumulative_dataframes_vd[i].iloc[:, 0] >= 800]

            # Add traces for each CSV file on the primary y-axis
            fig.add_trace(go.Scatter(x=filtered_dataframes_dvd.iloc[:, 0], y=filtered_dataframes_dvd[age_band], mode='lines',line=dict(dash='solid'), name=os.path.splitext(os.path.basename(csv_files_dvd[i]))[0]), secondary_y=False)
            fig.add_trace(go.Scatter(x=filtered_dataframes_vd.iloc[:, 0], y=filtered_dataframes_vd[age_band], mode='lines',line=dict(dash='dot'), name=os.path.splitext(os.path.basename(csv_files_vd[i]))[0]), secondary_y=False)
            # Add traces for the cumulative data on the secondary y-axis
            fig.add_trace(go.Scatter(x=filtered_cumulat_dataframes_dvd.iloc[:, 0], y=filtered_cumulat_dataframes_dvd[age_band], mode='lines', line=dict(dash='solid'), name=f'cumulative {os.path.splitext(os.path.basename(csv_files_dvd[i]))[0]}'), secondary_y=True)
            fig.add_trace(go.Scatter(x=filtered_cumulative_dataframes_vd.iloc[:, 0], y=filtered_cumulative_dataframes_vd[age_band], mode='lines', line=dict(dash='dot'), name=f'cumulative {os.path.splitext(os.path.basename(csv_files_vd[i]))[0]}'), secondary_y=True)
        ...
        fig.update_layout(
            ...
            yaxis=dict(
                title=f'Values per 100000 y1 DVD',
                side='left'                
            ),
            yaxis2=dict(
            ...
            yaxis3=dict(
                title=f"Values per 100000 Cumulative y3 DVD",
                overlaying="y",
                side="right",
                position=0.9                            
            ),
