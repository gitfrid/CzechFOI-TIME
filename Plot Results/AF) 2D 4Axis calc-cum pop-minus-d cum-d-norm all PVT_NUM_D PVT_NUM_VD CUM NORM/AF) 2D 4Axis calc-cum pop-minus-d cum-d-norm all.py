...
def main():
    ...
    for age_band in age_bands:  
        ...
        for i in range(0,len(dataframes_dvd)):            
            ...
            fig.add_trace(go.Scatter(x=cumulat_dataframes_dvd[i].iloc[:, 0], y=cumulat_dataframes_dvd[i][age_band], mode='lines', line=dict(dash='solid'), name=f'cumulative {os.path.splitext(os.path.basename(csv_files_dvd[i]))[0]}'), secondary_y=True)
